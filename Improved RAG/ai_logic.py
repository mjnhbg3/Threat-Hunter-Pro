...
Return ONLY the JSON object."""
    ...
    try:
        ...
        logging.info(f"DEBUG: Initial JSON parsing successful. Result type: {type(result)}, keys: {list(result.keys()) if isinstance(result, dict) else 'Not dict'}")
        
        # Handle wrapped response from Gemini API
        if "response" in result and isinstance(result["response"], str):
            try:
                logging.info(f"DEBUG: Unwrapping nested JSON response from AI. Original result keys: {list(result.keys())}")
                nested_response = result["response"]
                logging.info(f"DEBUG: Nested response content (first 500 chars): {nested_response[:500]}")
                result = json.loads(nested_response)
                logging.info(f"DEBUG: Successfully unwrapped nested JSON response. New result keys: {list(result.keys())}")
            except json.JSONDecodeError as e:
                logging.warning(f"DEBUG: Failed to parse nested response JSON: {e}")
                logging.warning(f"DEBUG: Problematic nested response: {result['response'][:1000]}")
                return {"security_issues": [], "operational_issues": []}
        
        # Validate structure
        if not isinstance(result, dict):
            logging.warning(f"DEBUG: Result is not a dictionary, it's {type(result)}: {result}")
            return {"security_issues": [], "operational_issues": []}
        
        security_issues = result.get("security_issues", [])
        operational_issues = result.get("operational_issues", [])
        
        logging.info(f"DEBUG: Extracted from result - security_issues type: {type(security_issues)}, length: {len(security_issues) if isinstance(security_issues, list) else 'N/A'}")
        logging.info(f"DEBUG: Extracted from result - operational_issues type: {type(operational_issues)}, length: {len(operational_issues) if isinstance(operational_issues, list) else 'N/A'}")
        
        if not isinstance(security_issues, list):
            logging.warning(f"DEBUG: security_issues is not a list, it's {type(security_issues)}: {security_issues}")
            security_issues = []
        if not isinstance(operational_issues, list):
            logging.warning(f"DEBUG: operational_issues is not a list, it's {type(operational_issues)}: {operational_issues}")
            operational_issues = []
        
        logging.info(f"DEBUG: Final comprehensive analysis found {len(security_issues)} security issues and {len(operational_issues)} operational issues")
        if security_issues:
            for i, issue in enumerate(security_issues):
                logging.info(f"DEBUG: Security issue {i+1}: {issue.get('title', 'No title')} with keys: {list(issue.keys()) if isinstance(issue, dict) else 'Not a dict'}")
        if operational_issues:
            for i, issue in enumerate(operational_issues):
                logging.info(f"DEBUG: Operational issue {i+1}: {issue.get('title', 'No title')} with keys: {list(issue.keys()) if isinstance(issue, dict) else 'Not a dict'}")
...
async def analyze_context_and_identify_issues(recent_logs: List[Dict[str, Any]]) -> None:
    ...
    try:
        ...
        logging.info(f"DEBUG: Comprehensive analysis found {len(comprehensive_results['security_issues'])} security issues and {len(comprehensive_results['operational_issues'])} operational issues")
        logging.info(f"DEBUG: Comprehensive results structure: security_issues={len(comprehensive_results.get('security_issues', []))}, operational_issues={len(comprehensive_results.get('operational_issues', []))}")
        
        # Debug: Log the actual results for troubleshooting
        if comprehensive_results['security_issues']:
            logging.info(f"DEBUG: Security issues found: {[issue.get('title', 'No title') for issue in comprehensive_results['security_issues']]}")
        if comprehensive_results['operational_issues']:
            logging.info(f"DEBUG: Operational issues found: {[issue.get('title', 'No title') for issue in comprehensive_results['operational_issues']]}")
...
IMPORTANT: Return ONLY the JSON object above with your analysis data filled in. Do not include any explanatory text, markdown formatting, or code blocks. Start your response with {{ and end with }}."""
        ...
        for issue in comprehensive_results["security_issues"]:
            ...
            logging.info(f"Added security issue to processing: {issue.get('title', 'No title')}")
        for issue in comprehensive_results["operational_issues"]:
            issue["category"] = "operational"
            identified_issues.append(issue)
            logging.info(f"Added operational issue to processing: {issue.get('title', 'No title')}")
        
        logging.info(f"DEBUG: Total issues to process: {len(identified_issues)}")
        if identified_issues:
            for i, issue in enumerate(identified_issues):
                logging.info(f"DEBUG: Issue {i+1}: {issue.get('title', 'No title')} - Category: {issue.get('category', 'No category')} - Fields: {list(issue.keys())}")
        
        existing_signatures: set[str] = set()
        for existing_issue in state.dashboard_data["issues"]:
            existing_signatures.add(generate_issue_signature(existing_issue))
        
        logging.info(f"Found {len(existing_signatures)} existing issue signatures")
        for issue_data in identified_issues:
            try:
                logging.info(f"DEBUG: Processing issue: {issue_data.get('title', 'No title')}")
                logging.info(f"DEBUG: Issue data keys: {list(issue_data.keys())}")
                logging.info(f"DEBUG: Issue data: {issue_data}")
                ...
                if not all(field in issue_data for field in required_fields):
                    ...
                    logging.warning(f"DEBUG: Issue missing required fields {missing_fields}: {issue_data}")
                    continue
                ...
                if issue_signature in existing_signatures:
                    logging.info(f"Skipping duplicate issue: {issue_data.get('title', 'No title')}")
                    continue
                issue_id = hashlib.sha256(issue_data['title'].encode()).hexdigest()[:10]
                if issue_id in state.ignored_issue_ids:
                    logging.info(f"Skipping ignored issue: {issue_data.get('title', 'No title')}")
                    continue
                ...
                logging.info(f"DEBUG: Successfully added issue to new_issues: {issue['title']} (ID: {issue['id']})")
            except Exception as e:
        ...
        await save_dashboard_data()
        logging.info(f"DEBUG: AI analysis complete. Found {len(truly_new_issues)} new unique issues.")
        logging.info(f"DEBUG: Final dashboard has {len(state.dashboard_data['issues'])} total issues")
        if truly_new_issues:
            for issue in truly_new_issues:
                logging.info(f"DEBUG: New issue added: {issue['title']} - Category: {issue.get('category', 'No category')}")
        else:
            logging.info("DEBUG: No new issues identified in this analysis cycle")
    except Exception as e:
        logging.error(f"Error during AI analysis: {e}")
        state.dashboard_data["summary"] = f"AI analysis failed: {e}"
        await save_dashboard_data()
