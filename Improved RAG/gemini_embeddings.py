...
class GeminiEmbeddingClient:
    """Client for Google Gemini Embedding API with proper rate limiting and fallback."""
    ...
    def __init__(self):
        ...
        self.rpm_limit, self.tpm_limit, self.rpd_limit = MODEL_QUOTA.get("embedding", (100, 30_000, 1_000))
    ...
    async def _find_available_key(self, estimated_tokens: int = 1000) -> Optional[str]:
        ...
        if not available_keys:
            logging.warning("All Gemini API keys have exceeded daily limits - triggering fallback to local embeddings")
        ...
        if current_key in available_keys:
            ...
            tmp_bucket = self._get_rate_bucket(current_key, "tpm")
        ...
        for i, key in enumerate(GEMINI_API_KEYS):
            ...
            if rpm_bucket.consume(1) and tpm_bucket.consume(estimated_tokens):
                ...
                logging.info(f"Switched to API key {i+1} for embedding request (daily count: {self.daily_request_counts[key]}/{self.rpd_limit})")
                return key
    ...
    async def embed_texts(self, texts: List[str], batch_size: int = 5) -> np.ndarray:
        ...
        Args:
            ...
            batch_size: Number of texts to process in each API call (reduced for rate limits)
        ...
        if self.all_keys_exhausted:
            logging.warning("Gemini API keys exhausted, cannot generate embeddings")
            return np.zeros((len(texts), self.dimension), dtype=np.float32)
        
        all_embeddings = []
        
        # Process texts in smaller batches for rate limiting
        for i in range(0, len(texts), batch_size):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if not api_key:
                        if self.all_keys_exhausted:
                            logging.warning(f"All API keys exhausted during batch processing. Processed {i}/{len(texts)} texts.")
            ...
            if i + batch_size < len(texts):
                await asyncio.sleep(0.5)  # Increased delay for stricter rate limits
